defmodule ConduitMQTTTest do
  ...
  defmodule StaticClientIdBroker do
    @moduledoc false
    def receives(_name, message) do
      Logger.info("StaticClientId broker recieved #{inspect(message)}")
      send(ConduitMQTTTest, {:broker, message})
...
#  end
  ...
  test "subscribers recovers from terminating and reconnects with same client_id" do

    subscribers = %{queue_static: [from: "static/bar1", qos: 2, client_id: "staticclientid"]}

    message =
      %Conduit.Message{}
      |> put_destination("static/bar1")
      |> put_body("test")


    opts = Application.get_env(:conduit, ConduitMQTTTest)

    {:ok, pid} = ConduitMQTT.start_link(StaticClientIdBroker, [], subscribers, opts)

    ConduitMQTT.Util.wait_until(fn -> ConduitMQTT.Meta.get_broker_status(StaticClientIdBroker) == :up end)
    Tortoise.Connection.disconnect("staticclientid")


    #ConduitMQTT.Meta.delete(StaticClientIdBroker)

    ConduitMQTT.publish(Broker, message, [], qos: 2, timeout: 100)

    #ConduitMQTT.start_link(StaticClientIdBroker, [], subscribers, opts)
